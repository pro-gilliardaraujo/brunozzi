...
def criar_mapa_base(centro_lat, centro_lon, dados_bounds=None):
    """Cria o objeto mapa base com camadas configuradas"""
    mapa = folium.Map(
    ...
    folium.TileLayer(
        ...
        overlay=False,
        control=True,
        show=True  # Satélite como padrão
    ).add_to(mapa)
    
    # Camada Mapa (OpenStreetMap)
    folium.TileLayer(
        tiles='OpenStreetMap',
        name='Mapa Rodoviário',
        overlay=False,
        control=True,
        show=False
    ).add_to(mapa)
...
def gerar_mapas_diarios(mapeamento_frotas, pasta_saida):
    """
    ...
    for dia_alvo in sorted(dias_disponiveis):
        ...
        areas = separating_areas = separar_por_clusters(dados_dia_filtrados)
        ...
        for area in areas:
            ...
            for idx, (frota_id, gdf_dia) in enumerate(sorted(dados_dia_filtrados.items())):
                ...
                grupo = folium.FeatureGroup(name=f"Frota {frota_id}", show=True)
                ...
                folium.PolyLine(
                    ...
                    popup=f"Frota {frota_id}<br>Dia: {str_dia}<br>Pontos: {len(coords_frota)}",
                ...
                folium.Marker(
                    ...
                    popup=f"<b>INÍCIO</b><br>Frota {frota_id}<br>{str_dia}"
                ).add_to(grupo)
                ...
                folium.Marker(
                    ...
                    popup=f"<b>FIM</b><br>Frota {frota_id}<br>{str_dia}"
                ).add_to(grupo)
            ...
            slider_html = f'''
                <div style="position: fixed; bottom: 30px; left: 30px; width: 220px; z-index:9999; 
                    ...
                    <label for="weight-slider" style="font-family: sans-serif; font-size: 12px;">Espessura da Linha: <span id="weight-val">{ESPESSURA_LINHA_PADRAO}</span></label>
                    <input type="range" id="weight-slider" min="1" max="10" value="{ESPESSURA_LINHA_PADRAO}" step="1" style="width: 100%;">
                </div>
                <script>
...
def main():
    ...
    # 3. Gerar mapas diários
    arquivos = gerar_mapas_diarios(mapeamento, PASTA_SAIDA)
    ...
    if arquivos:
        ...
        print("\n⚠️ Nenhum mapa gerado (possível falta de interseção de datas).")
