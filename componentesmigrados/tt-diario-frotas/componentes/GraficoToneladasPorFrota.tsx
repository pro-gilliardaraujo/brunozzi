...
export function GraficoToneladasPorFrota({ dados }: GraficoToneladasPorFrotaProps) {
  ...
  return (
    <div className="h-full flex items-center">
      {/* Container flexível que ocupa a altura disponível */}
      <div className="w-full h-full flex items-end justify-center gap-8">
        {dadosOrdenados.map((item) => {
          ...
          const valorBottom = percBottom + 13 + GAP_ENTRE_TEXTOS
          
          // A label tem posição fixa relativa ao fundo do container
          const labelBottom = PADDING_BOTTOM_LABEL

          return (
            // Removida altura fixa que forçava o container. Agora usa h-full para respeitar o pai.
            <div key={item.id} className="flex flex-col items-center h-full justify-end">
              {/* Container da coluna com largura fixa */}
              <div className="relative h-full w-full" style={{ width: `${larguraColunaPx}px` }}>
                ...
                <div
                  ...
                  style={{ width: `${larguraColunaPx - 8}px`, bottom: `${BASE_COLUNA}px`, height: `${alturaPx}px` }}
                />
                {/* Rótulo da frota fora da coluna, próximo à base (ligeiramente abaixo da base) */}
                <div
                  className="absolute left-1/2 -translate-x-1/2 text-xs font-bold text-black whitespace-nowrap"
