...
function RelatorioFrotasCd({
  ...
  const [frenteNomeStorage, setFrenteNomeStorage] = React.useState<
    string | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (savedMock) {
        ...
        if (typeof parsed.qtdFrotas === "number")
          setMockQtdFrotas(parsed.qtdFrotas);
        if (typeof parsed.qtdLavagem === "number")
          setMockQtdLavagemRows(parsed.qtdLavagem);
        if (typeof parsed.qtdRoletes === "number")
          setMockQtdRoletesRows(parsed.qtdRoletes);
    ...
    if (storedData) {
      try {
        ...
        if (parsed.coordenadas) {
          console.log(
            "Carregando mapa do localStorage",
            parsed.coordenadas.length,
            "pontos",
          );
  ...
  const dataFormatada =
    period === "semanal" ? `${startStr} - ${endStr}` : endStr;
  ...
  const [utilitiesPanelLeft, setUtilitiesPanelLeft] = React.useState<
    number | null
  >(null);
  ...
  const MAP_FRENTES: Record<string, string> = {
    frente5: "Frente BP Ituiutaba",
  };
  const frenteNome =
    frenteNomeStorage ||
    MAP_FRENTES[frenteCodigo] ||
    (frenteCodigo?.startsWith("Frente")
      ? frenteCodigo
      : frenteCodigo
        ? `Frente ${frenteCodigo}`
        : "Frente Desconhecida");
  ...
  const clampInt = React.useCallback(
  ...
  const totalFrotasBase = (
    Array.isArray(eficiencia_energetica)
      ? eficiencia_energetica.filter((d: any) => d?.nome)
      : []
  ).length;
  ...
  const buildRows = React.useCallback(
    <T,>(
      ...
      if (src.length === 0)
        return Array.from({ length: safeCount }, (_, idx) => makeFallback(idx));
  ...
  const handleDownloadPdf = React.useCallback(async () => {
    ...
    try {
      console.log("[PDF][CD-DIARIO] Exportação via Backend (Puppeteer/Local)", {
        filename,
      });
      ...
      toast({
        title: "PDF gerado",
        description: "Verifique a pasta de Downloads do navegador.",
      });
    } catch (e) {
  ...
  const computePageMetrics = React.useCallback(() => {
    ...
    const pages = Array.from(
      pagesRoot.querySelectorAll("[data-pdf-page]"),
    ) as HTMLDivElement[];
  ...
  React.useEffect(() => {
    ...
    const compute = () => {
      ...
      rafId = requestAnimationFrame(() => {
        const isCompact =
          document.documentElement.getAttribute("data-sidebar-compact") === "1";
        ...
        const panelWidth =
          panelEl.getBoundingClientRect().width || panelEl.offsetWidth || 0;
  ...
  const dadosValidosBase = React.useMemo(() => {
    const lista = Array.isArray(eficiencia_energetica)
      ? eficiencia_energetica
      : [];
  ...
  const buildNamedSeries = React.useCallback(
    (
      ...
      count: number,
      nameKey: "nome" | "Frota",
      makeFallback: (name: string, idx: number) => any,
    ) => {
      ...
      return Array.from({ length: safeCount }, (_, idx) => {
        ...
        if (typeof next.eficiencia === "undefined" || isNaN(next.eficiencia))
          next.eficiencia = isMocked ? Math.random() * 100 : 0;
        if (typeof next.horasMotor === "undefined" || isNaN(next.horasMotor))
          next.horasMotor = isMocked ? 5 + Math.random() * 15 : 0;
        if (
          typeof next.horasElevador === "undefined" ||
          isNaN(next.horasElevador)
        )
          next.horasElevador = isMocked ? 3 + Math.random() * 10 : 0;
        if (typeof next.velocidade === "undefined" || isNaN(next.velocidade))
          next.velocidade = isMocked ? 3 + Math.random() * 4 : 0;
        ...
        if (typeof next.percentual === "undefined" || isNaN(next.percentual))
          next.percentual = isMocked ? Math.random() * 20 : 0;

        // Garantir tempoManutencao também para o GraficoDisponibilidadeMecanica
        if (
          typeof next.tempoManutencao === "undefined" ||
          isNaN(next.tempoManutencao)
        )
          next.tempoManutencao = isMocked ? Math.random() * 2 : 0;
        ...
        if (
          typeof next.disponibilidade === "undefined" ||
          isNaN(next.disponibilidade) ||
          (isMocked && next.disponibilidade === 0)
        ) {
          if (next.horasMotor > 0) {
            const dispCalc = Math.max(
              0,
              ((next.horasMotor - (next.tempoManutencao || 0)) /
                next.horasMotor) *
                100,
            );
        ...
        if (typeof next.porcentagem === "undefined" || isNaN(next.porcentagem))
          next.porcentagem = isMocked ? 70 + Math.random() * 30 : 0;
        ...
        if (typeof next.tempoLigado === "undefined")
          next.tempoLigado = next.horasMotor;
        if (typeof next.tempoOcioso === "undefined")
          next.tempoOcioso = next.tempoLigado * (next.percentual / 100);
  ...
  const mediaVelocidadeFiltrada = React.useMemo(() => {
    ...
    return buildNamedSeries(base, qtdFrotasEfetivo, "nome", (name, idx) => ({
      id: `vel-${idx + 1}`,
      nome: name,
      velocidade: 0,
    }));
  ...
  const manobrasFiltradas = React.useMemo(() => {
    ...
    return buildNamedSeries(base, qtdFrotasEfetivo, "Frota", (name) => ({
      Frota: name,
      "Tempo Total": 0,
      "Tempo Médio (hh:mm)": "00:00:00",
  ...
  const disponibilidadeFiltrada = React.useMemo(() => {
    const base = Array.isArray(disponibilidade_mecanica)
      ? disponibilidade_mecanica
      : [];
    return buildNamedSeries(base, qtdFrotasEfetivo, "nome", (name, idx) => ({
      id: `disp-${idx + 1}`,
  ...
  const dadosUsoGPS = React.useMemo(() => {
    const base = Array.isArray(uso_gps)
      ? uso_gps.filter((d: any) => d?.nome)
      : [];
    return buildNamedSeries(base, qtdFrotasEfetivo, "nome", (name, idx) => ({
      id: `gps-${idx + 1}`,
      nome: name,
  ...
  const mediaEficiencia =
    dadosEficienciaNaoZero.reduce((acc, curr) => acc + curr.eficiencia, 0) /
    (dadosEficienciaNaoZero.length || 1);
  ...
  const dadosEficienciaOperacionalBase = React.useMemo(() => {
    const lista = Array.isArray(eficiencia_operacional)
      ? eficiencia_operacional
      : [];
    return lista.filter((d: any) => d?.nome);
  }, [eficiencia_operacional]);
  ...
  const dadosEficienciaOperacionalNaoZero =
    dadosEficienciaOperacionalValidos.filter((d) => d.eficiencia > 0);
  const mediaEficienciaOperacional =
    dadosEficienciaOperacionalNaoZero.reduce(
      (acc, curr) => acc + curr.eficiencia,
      0,
    ) / (dadosEficienciaOperacionalNaoZero.length || 1);
  ...
  const dadosHorasElevadorNaoZero = dadosValidos.filter(
    (d) => d.horasElevador > 0,
  );
  const mediaHorasElevador =
    dadosHorasElevadorNaoZero.reduce(
      (acc, curr) => acc + curr.horasElevador,
      0,
    ) / (dadosHorasElevadorNaoZero.length || 1);
  ...
  const producaoTotalValor =
    typeof producao_total?.[0]?.valor === "number"
      ? producao_total[0].valor
      : typeof producao === "number"
        ? producao
        : 0;
  ...
  const dadosUsoGPSNaoZero = dadosUsoGPS.filter(
    (d) => (d.porcentagem || 0) > 0,
  );
  const mediaUsoGPS =
    dadosUsoGPSNaoZero.reduce((acc, curr) => acc + curr.porcentagem, 0) /
    (dadosUsoGPSNaoZero.length || 1);
  ...
  const dadosResumo = dadosValidos.map((f) => {
    const nome = f.nome;
    const disp = (disponibilidadeFiltrada || []).find(
      (d: any) => d.nome === nome,
    );
    const ocioso = (motorOciosoFiltrado || []).find(
      (d: any) => d.nome === nome,
    );
    const gps = (dadosUsoGPS || []).find((d: any) => d.nome === nome);
    const vel = (mediaVelocidadeFiltrada || []).find(
      (d: any) => d.nome === nome,
    );
    const prod = (producao_por_frota || []).find((d: any) => d.nome === nome);
    const man = (manobrasFiltradas || []).find(
      (d: any) => String(d.Frota) === nome,
    );
    const elev = (horas_elevador || [])
  ...
  const dadosCardManobras = ((manobras_frotas || []) as Array<any>).map(
    (it: any) => {
      const medioStr = it?.["Tempo Médio (hh:mm)"];
      const medioNum = it?.["Tempo Médio"];
      const seconds =
        typeof medioStr === "string" && medioStr.length > 0
  ...
  const lavagemForTable = React.useMemo(() => {
    ...
    return buildRows<any>(base, mockQtdLavagemRows, (idx) => ({
      Data: endStr,
  ...
  const roletesForTable = React.useMemo(() => {
    ...
    if (!showMockControls) return base;
    return buildRows<any>(base, mockQtdRoletesRows, (idx) => ({
      Data: endStr,
  ...
  const roletesPaginated = React.useMemo(() => {
    if (roletesForTable.length <= 28)
      return { page1: roletesForTable, page2: [] };
  ...
  const layoutMode = React.useMemo(() => {
    ...
    const deveMoverMotor =
      qtdMotorOcioso > 7 || qtdLavagem > 10 || qtdRoletes > 2;
    ...
    <div className="relative bg-gray-100 p-1">
      ...
      <div className="flex items-start gap-4">
        <div className="flex-1 min-w-0">
          <div
            ref={scrollWrapRef}
            className="overflow-auto report-scroll flex justify-center"
          >
      ...
      */}
                {period === "diario" &&
                  (() => {
                    const totalPages = Math.ceil(
                      intervalosAgrupados.length / 4,
                    );
                    return Array.from({ length: totalPages }).map(
                      (_, pageIndex) => {
                        const startIndex = pageIndex * 4;
                        const pageItems = intervalosAgrupados.slice(
                          startIndex,
                          startIndex + 4,
                        );
                          ...
                          >
                            ...
                            >
                              ...
                              <div className="flex-1 flex flex-col overflow-hidden">
                                <SectionTitle
                                  title={`Intervalos de Operação${totalPages > 1 ? ` - página ${pageIndex + 1}` : ""}`}
                                />
                                ...
                                <div className="bg-slate-50 border border-slate-200 rounded p-2 mb-2 text-[10px] leading-tight text-slate-700">
                                  <p className="mb-4">
                                    <strong className="text-white bg-green-600 border border-green-600 shadow-[0_1px_3px_rgba(0,0,0,0.08)] px-2 py-1 rounded-sm">
                                      Produtivo:
                                    </strong>{" "}
                                    Referente aos apontamentos em efetivo, no
                                    caso de colhedoras o apontamento de colheita
                                    de cana.
                                  </p>
                                  <p className="mb-4">
                                    <strong className="text-white bg-blue-500 border border-blue-500 shadow-[0_1px_3px_rgba(0,0,0,0.08)] px-2 py-1 rounded-sm">
                                      Disponível:
                                    </strong>{" "}
                                    Todos os outros grupos de apontamento que
                                    não em manutenção: Manobra, aguardando
                                    transbordo, checklist, abastecimento, etc.
                                  </p>
                                  <p className="mb-4">
                                    <strong className="text-white bg-red-500 border border-red-500 shadow-[0_1px_3px_rgba(0,0,0,0.08)] px-2 py-1 rounded-sm">
                                      Manutenção:
                                    </strong>{" "}
                                    Tempo em parada pelo grupo de manutenção:
                                    corretiva, preventiva, elétrica, etc.
                                  </p>
                                  <p>
                                    <strong className="text-slate-600 bg-white border border-slate-300 shadow-[0_1px_3px_rgba(0,0,0,0.08)] px-2 py-1 rounded-sm">
                                      Falta de Informação:
                                    </strong>{" "}
                                    Tempo não registrado pela frota, por motivos
                                    como chave geral desligada ou problema de
                                    comunicação com bordo/gateway.
                                  </p>
                                </div>
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col gap-4 overflow-hidden pt-2">
                      <h2 className="text-center font-bold text-base">
                        Resumo do Relatório de Colheita {periodoLabel}
                      </h2>
      ...
      <div
        ...
        style={
          utilitiesPanelLeft == null ? undefined : { left: utilitiesPanelLeft }
        }
      >
        <div
                ...
                <div className="flex flex-col gap-2 border-t pt-2">
                  ...
                  <div className="grid grid-cols-[1fr_66px] items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setMockQtdFrotas(
                          clampInt(e.target.value, 1, MAX_MOCK_QTD_FROTAS),
                        )
                      }
                    />
                  </div>
                  <div className="grid grid-cols-[1fr_66px] items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setMockQtdLavagemRows(clampInt(e.target.value, 0, 50))
                      }
                    />
                  </div>
                  <div className="grid grid-cols-[1fr_66px] items-center gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setMockQtdRoletesRows(clampInt(e.target.value, 0, 50))
                      }
                    />
                  </div>
                </div>
...
export default function RelatorioDiarioFrotas({
  ...
  const period =
    propPeriod || (searchParams?.period === "semanal" ? "semanal" : "diario");
