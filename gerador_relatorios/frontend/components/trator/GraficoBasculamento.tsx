...
export function GraficoBasculamento({
  ...
  const getMedioSeconds = (item: ItemBasculamento): number => {
  ...
  const dadosOrdenados = [...dados].sort((a, b) => {
    const timeA = getMedioSeconds(a)
    const timeB = getMedioSeconds(b)
    return timeA - timeB
  })
  
  const maiorTempoMedio = Math.max(...dados.map(d => getMedioSeconds(d)), 0)
  ...
  const rowGapClass = isTight ? 'gap-1' : isSpacious ? 'gap-3' : 'gap-2'
    ...
    <div className="flex flex-col h-full">
      ...
      <div className={`flex flex-col ${rowGapClass} flex-1 overflow-hidden`}>
        {itensVisiveis.map((item, index) => {
          const valorSeconds = getMedioSeconds(item)
              ...
              <div className={`flex items-center ${compact ? "gap-1.5" : "gap-2"}`}>
                ...
                {isTight ? (
                  <div className="text-[10px] font-medium text-slate-600 whitespace-nowrap">
                    Tempo Total:{' '}
                    <span className="font-bold text-[10px] whitespace-nowrap" style={{ color: corItem }}>
                      {typeof item["Tempo Total (hh:mm)"] === 'string' && item["Tempo Total (hh:mm)"].length > 0
                        ? item["Tempo Total (hh:mm)"]
                        : hoursToHm(item["Tempo Total"] || 0)}
                    </span>
                  </div>
                ) : (
                  <div className={`flex flex-col items-center ${isTight ? 'w-16 min-w-[64px]' : 'w-20 min-w-[80px]'}`}>
                ...
                <div className={`${isTight ? 'text-xs' : isSpacious ? 'text-base' : 'text-sm'} font-bold w-16 text-right`} style={{ color: corItem }}>
                  {typeof item["Tempo Médio (hh:mm)"] === 'string' && item["Tempo Médio (hh:mm)"].length > 0
                    ? item["Tempo Médio (hh:mm)"]
                    : secondsToMmSs((item["Tempo Médio"] || 0) * 3600)}
                </div>
              </div>
            </div>
          )
