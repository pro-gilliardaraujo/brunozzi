...
export default function JsonReportPage({ params }: PageProps) {
  ...
  if (category === 'colhedora' && type === 'frotas' && period === 'diario') {
    // Detect unified JSON format (generated by 6_ConsolidarJSON.py)
    // Unified format has 'metadata' at the top level; legacy format has frota IDs as keys
    if (rawData.metadata && rawData.eficiencia_energetica) {
      // New unified format — pass directly, no transformation needed
      return <RelatorioFrotasCd period="diario" data={rawData} />;
    }

    // Legacy Solinftec-only format — use transformer
      ...
      <div className="max-w-4xl w-full space-y-6">
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
             <div className="bg-zinc-900/50 px-4 py-3 border-b border-zinc-800 flex justify-between items-center">
                ...
                <span className="text-xs text-zinc-600 font-mono">{params.file}</span>
             </div>
