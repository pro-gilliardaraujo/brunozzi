...
import { TratorOperadoresData, TratorOperadorData } from "@/lib/types"
...
export default function TratorOperadoresDiarioPage({ params }: PageProps) {
  const data = getReportByDate('tratores', 'operadores', params.data) as TratorOperadoresData | null
  ...
  const totalTransbordos = operadores.reduce((acc, op) => acc + op.Quantidade_Transbordos, 0)
  ...
  const mediaSemApontamento = operadores.reduce((acc, op) => acc + op.Porcentagem_Sem_Apontamento, 0) / (operadores.length || 1)
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
      <div className="max-w-[1400px] mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-zinc-100">
                Relatório Diário - Trator Operadores
              </h1>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4">
            <h3 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-2">Transbordos</h3>
            <p className="text-2xl font-bold text-blue-400">{totalTransbordos}</p>
          </div>
        </section>
            ...
            <table className="w-full text-xs">
              <thead className="bg-zinc-900/60">
                <tr>
                  ...
                  <th className="text-center p-3 font-semibold text-zinc-400">Transbordos</th>
              ...
              <tbody className="divide-y divide-zinc-800">
                {operadores.map((op, index) => (
                  <tr key={op.nome} className={`hover:bg-zinc-800/40 ${index % 2 === 0 ? 'bg-zinc-900/20' : ''}`}>
                    ...
                    <td className="p-3 text-center text-blue-400">{op.Quantidade_Transbordos}</td>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4">
            ...
            <div className="space-y-2">
              {operadores
                .filter(op => op.Quantidade_Transbordos > 0)
                .sort((a, b) => b.Quantidade_Transbordos - a.Quantidade_Transbordos)
                .slice(0, 5)
                .map((op) => (
                  <div key={op.nome} className="flex items-center justify-between bg-blue-900/20 border border-blue-900/30 rounded-lg p-3">
                    <span className="text-sm text-zinc-200">{op.nome}</span>
                    <div className="text-right">
                      <span className="text-sm font-bold text-blue-400">{op.Quantidade_Transbordos}</span>
                      <span className="text-xs text-zinc-500 ml-1">transbordos</span>
                    </div>
                  </div>
                ))}
              {operadores.filter(op => op.Quantidade_Transbordos > 0).length === 0 && (
                <p className="text-sm text-zinc-500 text-center py-4">Nenhum transbordo registrado</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4">
            <h3 className="text-sm font-semibold text-zinc-300 mb-4 flex items-center gap-2">
              ...
              Atenção - Falta de Apontamento
            </h3>
            <div className="space-y-2">
              {operadores
                .filter(op => op.Porcentagem_Sem_Apontamento > 15)
                .sort((a, b) => b.Porcentagem_Sem_Apontamento - a.Porcentagem_Sem_Apontamento)
                .slice(0, 5)
                .map((op) => (
                  <div key={op.nome} className="flex items-center justify-between bg-amber-900/20 border border-amber-900/30 rounded-lg p-3">
                    <span className="text-sm text-zinc-200">{op.nome}</span>
                    <span className="text-sm font-bold text-amber-400">{op.Porcentagem_Sem_Apontamento.toFixed(1)}%</span>
                  </div>
                ))}
              {operadores.filter(op => op.Porcentagem_Sem_Apontamento > 15).length === 0 && (
                <p className="text-sm text-zinc-500 text-center py-4">Nenhum operador com falta de apontamento acima de 15%</p>
              )}
            </div>
          </div>
        </section>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-zinc-900/60">
                <tr>
                  <th className="text-left p-3 font-semibold text-zinc-400">Operador</th>
                  <th className="text-center p-3 font-semibold text-zinc-400">Vel. Desl. Vazio (km/h)</th>
                  <th className="text-center p-3 font-semibold text-zinc-400">Vel. Desl. Carregado (km/h)</th>
                  <th className="text-center p-3 font-semibold text-zinc-400">Frotas</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800">
                {operadores.map((op, index) => (
                  <tr key={op.nome} className={`hover:bg-zinc-800/40 ${index % 2 === 0 ? 'bg-zinc-900/20' : ''}`}>
                    ...
                    <td className="p-3 text-center text-zinc-300">
                      {op.Vel_Desl_Vazio_media ? op.Vel_Desl_Vazio_media.toFixed(2) : '-'}
                    </td>
                    <td className="p-3 text-center text-zinc-300">
                      {op.Vel_Desl_Carregado_media ? op.Vel_Desl_Carregado_media.toFixed(2) : '-'}
                    </td>
                    <td className="p-3 text-center text-zinc-400 text-[10px]">{op.Frotas_no_dia}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <footer className="border-t border-zinc-800 pt-4 mt-8">
          <div className="flex items-center justify-between text-xs text-zinc-600">
            <span>Relatório de Trator Operadores - {formatDateForDisplay(params.data)}</span>
