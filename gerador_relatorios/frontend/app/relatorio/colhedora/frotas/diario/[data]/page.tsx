...
import { GraficoHorasElevador } from "@/components/colhedora"
import { MapaAreaTrabalhada } from "@/components/colhedora"
...
export default function ColhedoraFrotasDiarioPage({ params }: PageProps) {
  ...
  const { metadata, metas, eficiencia_energetica, horas_elevador, motor_ocioso, 
          manobras_frotas, media_velocidade, disponibilidade_mecanica, 
          intervalos_operacao, ofensores, imagens, producao, producao_total } = data
  ...
  // Prepare data for hours elevador chart - uses 'horas' property
  const dadosHorasElevador = eficiencia_energetica.map(d => ({
    id: d.id,
    nome: d.nome,
    horas: d.horasElevador
  }))

  // Prepare data for manobras chart - requires all properties including formatted strings
  ...
  const dadosResumo = eficiencia_energetica.map(f => {
    ...
    const elev = horas_elevador.filter(d => d.nome === f.nome).reduce((acc, curr) => acc + curr.valor, 0)

    return {
      frota: f.nome,
      eficiencia: f.eficiencia || 0,
      horasElevador: elev || f.horasElevador || 0,
      producao: 0, // Would need producao_por_frota
      velocidade: vel?.velocidade || 0,
      gps: 0, // Would need uso_gps
  ...
  // Use the Intervalo type from GraficoIntervalos component
  type Intervalo = {
  ...
  const producaoTotalValor = producao_total?.[0]?.valor || producao || 0
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-zinc-100">
                Relatório Diário - Colhedora Frotas
              </h1>
        ...
        {imagens?.areaTrabalhada && (
          <section className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4">
            ...
            <MapaAreaTrabalhada imagemBase64={imagens.areaTrabalhada} tipo="imagem" />
          </section>
        )}
        ...
        <footer className="border-t border-zinc-800 pt-4 mt-8">
          <div className="flex items-center justify-between text-xs text-zinc-600">
            <span>Relatório de Colhedora Frotas - {formatDateForDisplay(params.data)}</span>
